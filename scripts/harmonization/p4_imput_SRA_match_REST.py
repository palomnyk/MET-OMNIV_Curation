...
if os.path.exists(result_fpath):
    ...
    start_flag = result_df.shape[0]

print(f"start flag {start_flag}, query len {dietary_data.shape[0]}")
assert(start_flag < dietary_data.shape[0] + 1)
...
for f_item in range(start_flag,dietary_data.shape[0]):
    ...
    query_desc = dietary_data["item"][f_item].upper()
    time.sleep(0.4)
    try:  
        data = query({
            "inputs": {
